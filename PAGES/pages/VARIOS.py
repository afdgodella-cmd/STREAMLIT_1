...
def H2():
    ...
    curs.execute("select * from varios order by fecha")
    data = curs.fetchall()
    
    # Obtener los nombres de las columnas
    #columnas = [desc[0] for desc in curs.description]
    columnas =['Fecha         ','Estudios estabilidad fuera plazo','Paradas producción debidas a QA/QC','Accidentes']
    # Crear un DataFrame
...
def main():
    ...
    with col12:    
      estabilidad = st.number_input("Estabilidad fuera plazo", step=1, format="%i")
      if st.button('Añadir'):
             accion ='Añadir'     
    with col13:
        paradas = st.number_input("Paradas producción", step=1, format="%i")
        if st.button('Corregir'):
             accion ='Corregir'
    with col14:
        accidentes= st.number_input("Accidentes", step=1, format="%i")
    ...
    if accion == 'Añadir':   
         conexion =[datosPSS.url,str(d), estabilidad, paradas, accidentes]
         Funciones_H2.añadeV(conexion)
         st.rerun()
    if accion == 'Corregir':        
         conexion =[datosPSS.url,str(d), estabilidad, paradas, accidentes]
         Funciones_H2.corrigeV(conexion)
         st.rerun()
    if accion == 'Eliminar':        
         conexion =[datosPSS.url,str(d), estabilidad, paradas, accidentes]
         Funciones_H2.eliminaV(conexion)
         st.rerun()
